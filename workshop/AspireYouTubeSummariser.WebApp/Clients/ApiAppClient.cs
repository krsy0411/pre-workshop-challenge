...
public interface IApiAppClient
{
    ...
    Task<string> MelonChartAsync(string question);
}
...
public class ApiAppClient(HttpClient http) : IApiAppClient
{
    ...
    public async Task<string> MelonChartAsync(string question)
    {
        using var response = await _http.PostAsJsonAsync("melonchart", new { question }).ConfigureAwait(false);
